...
- terminal-loop smoke now also covers the command-only repeated-stall branch, so if a typed terminal task keeps rerunning the same verification command result, runtime is expected to break out of that loop, inspect the hidden verifier file, repair it, and rerun the command within the same task
...
- terminal-loop smoke now also covers the combined `assistant-pass + tool-recovery + command-only verification` chain, so a task can burn passes on progress-only replies, then survive invalid edit tool input, and still return to the pending `npm test` flow inside the same task
- terminal-loop smoke also covers the command-only `assistant-pass + tool-recovery` terminal chain end to end, so the real typed terminal path is protected not just for simple handoffs but for the full “progress-only loop -> invalid tool input -> hidden verification file repair -> rerun command” sequence
