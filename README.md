...
- when a task hits the tool-step ceiling before the next `files` / `edit` / `write` action executes, runtime now records that pending target so a later `继续` can resume from the blocked step instead of restarting broad exploration
