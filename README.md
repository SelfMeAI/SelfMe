...
- terminal-loop smoke now also covers the approval-wait branch: a typed project task can be interrupted while a later edit approval is still pending, and a typed `还能继续吗` resume is expected to jump straight back to that pending edit instead of reopening the project entry or the already-read work file
